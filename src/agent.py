...
class RAGAgent:
    ...
    #     try:
    #         self.logger.info("Invoke called | question=%r", question)
    #         t0 = time.perf_counter()
    #         response = self.graph.invoke({
    #             "messages": [HumanMessage(content=question)]
    #         })
    #         self.logger.info("Graph invoke completed in %.2f ms", (time.perf_counter() - t0) * 1000)

    #         msgs = response["messages"]
    #         self.logger.debug("Invoke: total messages in trace=%d", len(msgs))

    #         final_ai = next(
    #             (m for m in reversed(msgs) if m.type == "ai" and not getattr(m, "tool_calls", None)),
    #             msgs[-1]
    #         )
    #         final_text = final_ai.content if hasattr(final_ai, "content") else str(final_ai)

    #         out_dir = Path("output"); out_dir.mkdir(parents=True, exist_ok=True)
    #         ist = pytz.timezone("Asia/Kolkata")
    #         now = datetime.now(ist).strftime("%Y-%m-%d_%H-%M-%S")
    #         output_filename = f"{now}.json"
    #         trace_path = out_dir / output_filename

    #         trace = {"question": question, "messages": [message_to_dict(m) for m in msgs]}
    #         with open(trace_path, "w", encoding="utf-8") as f:
    #             json.dump(trace, f, indent=2, ensure_ascii=False)
    #         self.logger.info("Trace saved to %s", trace_path)

    #         return final_text
    #     except Exception:
    #         self.logger.exception("invoke failed for question=%r", question)
    #         raise
    ...
    def invoke(self, question: str) -> str:
        ...
        final_ai = next((m for m in reversed(msgs) if m.type == "ai" and not getattr(m, "tool_calls", None)), msgs[-1])
        return final_ai.content if hasattr(final_ai, "content") else str(final_ai)

if __name__ == "__main__":
    setup_logger()

    agent = RAGAgent(knowledge_base="policies_db.json")
